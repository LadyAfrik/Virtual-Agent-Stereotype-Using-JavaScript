...
def dashboard_page(request):
    """Render the dashboard template with user engagement metrics."""
    # Calculate total users and percentage who watched all videos
    total_users = Users.objects.count()
    watched_all = Users.objects.filter(watched_the_videos=1).count()
    percentage_watched_all = (watched_all / total_users) * 100 if total_users > 0 else 0
    remaining_percentage = 100 - percentage_watched_all  # Calculate the remaining here

    # Pass the calculated data to the template
    context = {
        'percentage_watched_all': percentage_watched_all,
        'remaining_percentage': remaining_percentage,  # Pass remaining value
    }
...
class AttributeRankingAPIView(APIView):
    """API to get all attribute rankings or filter by agent_name and/or category."""
    def get(self, request):
        agent_name = request.query_params.get('agent_name', None)
        category = request.query_params.get('category', None)

        # Filter rankings if query parameters are provided
...
class UsersAPIView(APIView):
    """API to get all users or filter by gender and/or level_of_study."""
    def get(self, request):
        gender = request.query_params.get('gender', None)
        level_of_study = request.query_params.get('level_of_study', None)

        # Filter users if query parameters are provided
...
class GenderSelectionAPIView(APIView):
    """API to get all gender selections or filter by agent_name and/or selected_gender."""
    def get(self, request):
        agent_name = request.query_params.get('agent_name', None)
        selected_gender = request.query_params.get('selected_gender', None)

        # Filter gender selections if query parameters are provided
...
def dashboard_page(request):
    """Render the dashboard template with all metrics and gender predictions."""

    # User Engagement Metrics
    total_users = Users.objects.count()
    watched_all = Users.objects.filter(watched_the_videos=1).count()
    percentage_watched_all = round((watched_all / total_users) * 100, 0) if total_users > 0 else 0
    remaining_percentage = 100 - percentage_watched_all
    ...
    # Combine all metrics into context
    context = {
        # User Engagement Metrics
    ...
    """Render the dashboard template with metrics, gender predictions, and attribute rankings."""

    # Fetch distinct attributes
    distinct_attributes = AttributeRanking.objects.values('attribute').distinct()

    # Prepare attribute rankings data for the box plot
